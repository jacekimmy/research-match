    ...
    <main className="rm-page" style={{ maxWidth: "800px" }}>
      ...
      <form onSubmit={handleSubmit} className="glass-card" style={{ padding: "28px 32px", marginBottom: "40px" }}>
        <textarea
          ...
          style={{
            width: "100%", minHeight: "100px", background: "rgba(255,255,255,0.5)",
            border: "1.5px solid rgba(186,192,149,0.35)", borderRadius: "14px",
            padding: "16px 20px", fontSize: "1rem", color: "#3D4127",
            fontFamily: "'Playfair Display', Georgia, serif", resize: "vertical",
        ...
        <div style={{ display: "flex", gap: "12px", marginTop: "16px", flexWrap: "wrap", alignItems: "center" }}>
          <select
            ...
            style={{
              padding: "10px 16px", fontSize: "0.9rem", borderRadius: "10px",
          ...
          <input
            ...
            style={{
              ...
              border: "1.5px solid rgba(186,192,149,0.35)", background: "rgba(255,255,255,0.5)",
              color: "#3D4127", fontFamily: "'Playfair Display', Georgia, serif",
              outline: "none", flex: 1, minWidth: "150px",
            }}
          />
          <button type="submit" disabled={!content.trim() || submitting} className="btn-cta" style={{ padding: "10px 28px", fontSize: "0.95rem" }}>
            {submitting ? "Posting..." : "Post"}
          </button>
        </div>
        {submitted && (
          <p style={{ marginTop: "12px", fontSize: "0.9rem", color: "#2d5a3d", fontWeight: 600 }}>
            ...
            <div key={item.id} className="glass-card card-enter" style={{ padding: "24px 28px", display: "flex", gap: "20px", alignItems: "flex-start", animationDelay: `${idx * 0.06}s` }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: hasVoted ? "rgba(45,90,61,0.1)" : "rgba(186,192,149,0.15)",
                  border: `1.5px solid ${hasVoted ? "rgba(45,90,61,0.3)" : "rgba(186,192,149,0.3)"}`,
                  borderRadius: "12px", padding: "10px 14px", cursor: hasVoted ? "default" : "pointer",
                  transition: "all 0.3s ease", minWidth: "52px",
                }}
                onMouseEnter={(e) => { if (!hasVoted) { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 4px 12px rgba(45,90,61,0.1)"; } }}
                onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; }}
              >
                <span style={{ fontSize: "1rem", color: hasVoted ? "#2d5a3d" : "#8A8D72", transition: "color 0.2s" }}>&#9650;</span>
                <span style={{ fontSize: "0.95rem", fontWeight: 700, color: hasVoted ? "#2d5a3d" : "#3D4127", fontFamily: "'Playfair Display', Georgia, serif" }}>{item.upvotes}</span>
              </button>

              {/* Content */}
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: "1rem", color: "#3D4127", lineHeight: 1.6 }}>{item.content}</p>
      ...
      {!loading && items.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px 20px" }}>
          ...
          <p style={{ fontSize: "1.1rem", color: "#8A8D72" }}>No feedback yet. Be the first!</p>
        </div>
      )}
    </main>
  );
